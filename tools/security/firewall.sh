...
# -----------------------------------------------------------------------------
# 运行方式
#   本地：cd /opt/netool && ./main.sh firewall
#   远程：bash <(curl -fsSL https://gitee.com/suser747/netool/raw/master/main.sh) firewall
#   直接：bash tools/security/firewall.sh [参数...]  （需在 tools 目录结构完整时）
# -----------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
main() {
  netool_acquire_lock "firewall" "另一个 firewall 实例正在运行，请等待其完成后再试。" || return 1
